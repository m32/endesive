...
"""
    ...
    def makepdf(self, pdfdata1, udct, zeros):
        ...
        page = (
            udct.get(b"sigpage", 0) if 0 <= udct.get(b"sigpage", 0) <= pages - 1 else 0
        )
        ...
        objs = [
            self.makeobj(page, (b"/Annots[%s %d 0 R]" % (annots, no + 3) + pagedata)),
            self.makeobj(no + 0, infodata),
            self.makeobj(no + 1, (b"/AcroForm %d 0 R" % (no + 2)) + rootdata),
            self.makeobj(
                no + 2,
                b"/Fields[%s %d 0 R]/SigFlags %d" % (fields, no + 3, udct[b"sigflags"]),
            ),
            visualization,
            self.makeobj(
                nav + 1,
...
"""
    ...
    def sign(self, datau, dct, key, cert, othercerts, algomd, hsm, timestampurl):
        ...
        contents = signer.sign(
            None, key, cert, othercerts, algomd, True, md, hsm, False, timestampurl
        )
