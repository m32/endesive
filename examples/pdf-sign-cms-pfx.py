...
def create_args():
    ...
    parser.add_argument(
        "pfx_certificate",
        type=str,
        help="Specify keystore file in .pfx format (Mandatory)",
    )
    parser.add_argument(
        "password", type=str, help=" Specify password for keystore file (mandatory)"
    )
    parser.add_argument(
        "src",
        type=str,
...
def validate_args(args):
    ...
    IS_PFX = lambda pfx_certificate: re.match(
        r"^(.[^,]+)(.pfx|.PFX|.p12|.P12){1}$", pfx_certificate
    )
